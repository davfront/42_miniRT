...
int	rt_get_cylinder_hit(t_ray ray, t_obj *obj, t_float t_max, t_hit *hit)
{
	...
	AO = vec3_subtract(ray.pos, cylinder.bot);
	// AOxAB cross product between AO and AB
	AOxAB = vec3_cross(AO, cylinder.vec_height);
	// VxAB V is ray direction
	VxAB = vec3_cross(ray.dir, cylinder.vec_height);
	// quadratic coef a, b, c
	a = vec3_dot(VxAB, VxAB);
	b = vec3_dot(VxAB, AOxAB) * 2;
	c = vec3_dot(AOxAB, AOxAB) - (obj->cylinder.radius * obj->cylinder.radius * cylinder.vec_h_square);
	// discriminent
	d = b * b - 4 * a * c;
	t1 = INFINITY;
	t2 = INFINITY;
	...
	if (d >= 0)
	{
		...
		projection = vec3_add(cylinder.bot, vec3_scale(cylinder.vec_height, vec3_dot(cylinder.vec_height, vec3_subtract(intersection, cylinder.bot)) / cylinder.vec_h_square)); 
		if ((vec3_length(vec3_subtract(projection, cylinder.bot)) +\
		vec3_length(vec3_subtract(projection, cylinder.top))) > vec3_length(cylinder.vec_height))
			t1 = INFINITY;
	}
	if (!hit)
		return (1);
