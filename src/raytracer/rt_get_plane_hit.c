...
/*   Updated: 2023/07/10 15:52:06 by dapereir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

static t_plane	rt_transformed_plane(t_mat4 mt)
{
	t_plane	pl;

	pl.point = mat4_multiply_vec3(mt, vec3(0, 0, 0));
	pl.normal = mat4_multiply_axis(mt, vec3(0, 1, 0));
	return (pl);
}
...
int	rt_get_plane_hit(t_ray ray, t_obj *obj, t_float t_max, t_hit *hit)
{
	...
	if (!obj || obj->type != PLANE)
		return (0);
	plane = rt_transformed_plane(obj->mt);
